#To create a dircord bot, importing the discord library is neccesary to create the bot.
#The os library is used to interact with the linux operating system and the environment variablbes in the ec2 instance.
#The import random is used to generate a random choice or number such as choosing a randoom response later in the code.
#To retrieve the metadata from the the Amazon instance, we need to import from ecs_metadata.
#To import the the variables from a separate file named '.env', we load and import using loud_fotenv.
import discord 
import os 
import random 
...
#I created a list containing three senctences and defined it as 'jokes'.
jokes = [
...
#To load the .env file containing the discord bot token, I use the load_dotenv()
#To create the discod bot client, I use dicord.bot()
load_dotenv()
client = discord.Bot()
token = str(os.getenv('TOKEN')) #This retrienve the token from the .env file and converts it into a string.
...
#this also retrieves informations such as username, channel name, and what the message is for the instance terminal.
@client.event
async def on_message(message):
    ...
    if channel == "random" or "bot-rando" or "general":
        #The try and else blocks are use as an error handler to give an error message if the bot cannot connect or loses connection from the server.
        try:
            ...
            elif user_message.lower() == "tell me about my server":#If the bot recieves this message, it pull information from the ec2 metadata and responds accordingly.
            ...
            #This handle any other inputs not matching the conditions above
